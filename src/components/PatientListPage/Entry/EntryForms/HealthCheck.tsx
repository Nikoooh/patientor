...
const HealthCheckForm = ({setType, healthCheckFormRef, handleNewEntry, diagnoses, entryDiagnoses, setEntryDiagnoses}: EntryFormProps): JSX.Element => {
  ...
  return (
    <form onSubmit={handleNewEntry} style={{marginTop: '20px', borderTop: '1px solid rgba(192,192,192,0.4)', borderRadius: '2px', outline: 'none'}} ref={healthCheckFormRef}>
      <Typography sx={{mt: 2}} variant="h5">Type: HealthCheck</Typography>
      ...
      <Typography variant="h6" sx={{mt: 2}}> Healthcheck Rating </Typography>    
      <FormControl sx={{width: '35vw', mt: 3}}>
        <InputLabel id='diagnosis-selector-label'>Health Rating</InputLabel>
          <Select name='healthRating' value={healthRating} id='healthrating-selector' labelId='healthrating-select' label='healthrating' onChange={({ target }) => setHealthRating(Number(target.value))}>             
